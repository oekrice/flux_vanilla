...
CONTAINS
  ...
  SUBROUTINE output_routines(i)   ! i=step index
    ...
    IF (print_arrays) THEN   ! output a snapshot of arrays   
       ...
       WRITE(50) time
       WRITE(50) rho(1:nx,1:ny,1:nz), ion_mass * (gamma - 1.0_num) * (energy(1:nx,1:ny,1:nz) &
                   - (1.0_num - xi_n(1:nx,1:ny,1:nz)) * ionise_pot) / (2.0_num - xi_n(1:nx,1:ny,1:nz))  
       ...
       WRITE(50) parallel_current(1:nx,1:ny,1:nz), perp_current(1:nx,1:ny,1:nz),eta_perp(1:nx,1:ny,1:nz)
       CLOSE(50)
#else       
       ...
       data = ion_mass * (gamma - 1.0_num) * (energy(0:nx,0:ny,0:nz) &
                   - (1.0_num - xi_n(0:nx,0:ny,0:nz)) * ionise_pot) / (2.0_num - xi_n(0:nx,0:ny,0:nz))
       CALL MPI_FILE_WRITE_ALL(filehandle,data, localcellcount, mpireal, status, errcode)
