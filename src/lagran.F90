    ...
    IF (resistiveMHD) THEN
       ...
       DO WHILE (keepcycling)
	     ...
	     keepcycling = .FALSE.
          ...
          IF (include_neutrals) CALL neutral_fraction
          IF (partially_ionized) CALL perpendicular_resistivity
          CALL set_dt
          substeps = substeps + 1
       ENDDO
       dt = actual_dt
...
!DEC$ VECTOR ALWAYS
  ...
  SUBROUTINE viscosity_and_b_update
    ...
    p_visc = 0.0_num
    pressure = (energy(-1:nx+2,-1:ny+2,-1:nz+2) - (1.0_num - xi_n(-1:nx+2,-1:ny+2,-1:nz+2)) * ionise_pot)&
...
END SUBROUTINE viscosity_and_b_update
  ...
  SUBROUTINE resistive_effects
    ...
    REAL(num) :: magn_j_par, magn_j_perp,flip,PI,NPI
...
!PI and NPI are flags used to switch between partially ionised and fully ionised
    IF (partially_ionized) THEN
       PI=1.0_num
       NPI=0.0_num
    ELSE
       PI=0.0_num
...
!	     If magn_b**2 < none_zero j_par_{x,y,z}=0.0_num
             j_par_x = (jx*bxv + jy*byv + jz*bzv) * bxv /MAX(magn_b**2,none_zero)
             j_par_y = (jx*bxv + jy*byv + jz*bzv) * byv /MAX(magn_b**2,none_zero)
             j_par_z = (jx*bxv + jy*byv + jz*bzv) * bzv /MAX(magn_b**2,none_zero)
             ...
             ! it's heat not flux but:
             flux = flux + (dt * eta(ix,iy,iz) * curlb(ix,iy,iz)**2 / 8.0_num)*NPI ! eta j^2 at vertex 
	     ...
	     w2 = w2 * masky(iy)
             ...
             flux = (-((j_par_x * eta(ix,iy,iz)) + &
                  (j_perp_x*eta_perp(ix,iy,iz))) * dxc(ix) * dt / 2.0_num)*PI
             flux = flux-(jx * eta(ix,iy,iz) * dxc(ix) * dt / 2.0_num)*NPI
             ...
             flux = (-((j_par_y * eta(ix,iy,iz)) + &
                  (j_perp_y * eta_perp(ix,iy,iz))) * dyc(iy) * dt / 2.0_num)*PI
             flux = flux-(jy * eta(ix,iy,iz) * dyc(iy) * dt / 2.0_num )*NPI
             ...
             bz1(ix,iyp,iz) = bz1(ix,iyp,iz) + flux
             bz1(ixp,iyp,iz) = bz1(ixp,iyp,iz) - flux
                flux = (-((j_par_z * eta(ix,iy,iz)) + &
                     (j_perp_z * eta_perp(ix,iy,iz))) * dzc(iz) * dt / 2.0_num)*PI
                flux = flux-(jz * eta(ix,iy,iz) * dzc(iz) * dt / 2.0_num)*NPI
             by1(ix,iy,iz) = by1(ix,iy,iz) - flux
             by1(ixp,iy,iz) = by1(ixp,iy,iz) + flux
             by1(ix,iy,izp) = by1(ix,iy,izp) - flux
             by1(ixp,iy,izp) = by1(ixp,iy,izp) + flux
