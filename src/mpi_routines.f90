...
! Calculate initial displacement value: nx,ny,nz,(xb,yb,zb,time)*size of float
    ...
    ALLOCATE(eta_perp(-1:nx+2,-1:ny+2,-1:nz+2),bzone(-1:nx+2,-1:ny+2,-1:nz+2))
    ALLOCATE(parallel_current(0:nx,0:ny,0:nz), perp_current(0:nx,0:ny,0:nz))
    ...
    ALLOCATE(rho_ref(-1:nz_global+2), T_ref(-1:nz_global+2),tau_ref(-1:nz_global+2))
    ...
    eta_perp = 0.0_num 
    parallel_current = 0.0_num
    perp_current = 0.0_num
  ...
  SUBROUTINE mpi_close
    ...
    DEALLOCATE(eta, curlb, eta_perp, bzone)
    ...
    DEALLOCATE(parallel_current, perp_current, xi_n)
    DEALLOCATE(rho_ref, T_ref, tau_ref)
