...
CONTAINS
  ...
  SUBROUTINE damp_boundaries
    ...
    IF (damping) THEN  
!
! Here I assume damping is only in z direction
! Damping region starts at a distance length_z/10 away from ends
!
       DO ix = -2, nx+2
          Do iy = -2, ny+2
             DO iz = -2, nz+2
                alpha = 0.5_num*(1.0_num-tanh((zc(iz)-150.0_num)/20.0_num))
!                a = length_z/10.0_num
!                IF (zc(iz) .LT. a) THEN
!                  alpha=(1.0_num -(a-zc(iz))/a)
!                END IF
!                IF (zc(iz) .GT. 9.0_num*a) THEN
!                  alpha=(1.0_num +(9.0_num*a-zc(iz))/a)
!                END IF
                vx(ix,iy,iz) = alpha * vx(ix,iy,iz)
                vy(ix,iy,iz) = alpha * vy(ix,iy,iz)
                vz(ix,iy,iz) = alpha * vz(ix,iy,iz)
                IF (xc(ix) .GT. 115.0_num) THEN
                   a = 0.5_num*(1.0_num-tanh((xc(ix)-115.0_num)/20.0_num))
                   vx(ix,iy,iz) = a * vx(ix,iy,iz)
                   vy(ix,iy,iz) = a * vy(ix,iy,iz)
                   vz(ix,iy,iz) = a * vz(ix,iy,iz)
                END IF
                IF (xc(ix) .LT. -115.0_num) THEN
                   a = 0.5_num*(1.0_num+tanh((xc(ix)+115.0_num)/20.0_num))
                   vx(ix,iy,iz) = a * vx(ix,iy,iz)
                   vy(ix,iy,iz) = a * vy(ix,iy,iz)
                   vz(ix,iy,iz) = a * vz(ix,iy,iz)
