...
! NB: as there are now 2 ghost celss so indexing will fail if (nx,ny,nz)<MAX(2,offset)
  ...
  REAL(num), DIMENSION(:,:,:), ALLOCATABLE :: parallel_current, perp_current, xi_n
  ...
  REAL(num), DIMENSION(:), ALLOCATABLE :: rho_ref, T_ref, tau_ref
  ...
  REAL(num) :: dtr, eta_background, ion_mass, ionise_pot
  ...
  LOGICAL :: resistiveMHD, any_open, restart, obs, partially_ionized, newton_cooling
  LOGICAL :: include_neutrals,farfield,damping,deckfile

! data for partially ionised plasma
  REAL(num), PARAMETER :: f_bar = 6.210864258e26_num, T_bar=24.58222741_num
  REAL(num), PARAMETER :: Tr = 1.1263_num, r_bar = 6.457013e23_num
  REAL(num), PARAMETER :: Tr_bar = 1.0_num / Tr 
...
!Toriodal problem extra inputs
  REAL(num) :: B0,db, Bamp,twist

  ! My (Oliver's) extra stuff to deal with reading in and out, plus diagnostics
  INTEGER::  ndiags, nsnaps
