...
SAVE 
  INTEGER,PARAMETER :: ControlBlockElements = 35
  ...
  CHARACTER(len=30),DIMENSION(ControlBlockElements) :: ControlBlockName =(/"nx","ny","nz","nprocx","nprocy","nprocz","x_stretch","y_stretch","z_stretch",&
       ...
       "visc1","visc2","visc3","resistivemhd","j_max","eta0","eta_background","dt_multiplier","gamma","obs","include_neutrals","partially_ionized","newton_cooling",&
...
CONTAINS
  ...
  FUNCTION HandleControlDeck(Element,Value)
    ...
    CASE(28)
       obs=AsLogical(Value,HandleControlDeck)
    CASE(29)
       include_neutrals=AsLogical(Value,HandleControlDeck)
    CASE(30)
       partially_ionized=AsLogical(Value,HandleControlDeck)
    CASE(31)
       newton_cooling=AsLogical(Value,HandleControlDeck)
    CASE(32)
